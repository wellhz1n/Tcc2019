...
?>
<nav style=" background-color:<?php if (!empty($cor)) {
                                echo $cor;
                              } ?> " class="navbar <?php
                                                                                    if ($at != 1 && empty($cor)) {
                                                                                      echo 'navcor1';
        ...
        <?php } else { ?>
          <li class="nav-item dropdown bordaM bordaMFinal itemMenu">
            ...
            <div class="dropdown-menu" style="background: #0e0033 !important;" aria-labelledby="dropdownContaNormal">
              ...
              <a class="dropdown-item" href="?page=carteira">R$:<?php $money = mysqli_query($cx, "SELECT * FROM carteira where idUsuario = '{$_SESSION['id']}'");
